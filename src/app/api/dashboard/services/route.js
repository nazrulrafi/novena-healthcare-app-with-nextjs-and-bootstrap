...
export async function GET() {
    try {
        const servicesPage = await prisma.page.findFirst({
            where: { metaKey: 'services_page' },
            include: {
                pagemeta: {
                    orderBy: {
                        id: 'asc',
        ...
        if (!servicesPage) {
            return NextResponse.json({ message: 'Services page not found' }, { status: 404 });
        }

        return NextResponse.json({ message: 'success', data: servicesPage.pagemeta  });
    } catch (error) {
...
export async function PUT(req) {
    try {
        ...
        revalidatePath('/dashboard/pages/services');
